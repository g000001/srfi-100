...
;;; 	 (error 'define-lambda-object "too many arguments" z)))))
;;;
;;; (define-function (check-duplicate ls err-str)
;;;   (cond ((null? ls) :false)
;;; 	((memq (car ls) (cdr ls)) (error 'define-lambda-object err-str (car ls)))
;;; 	(:else (check-duplicate (cdr ls) err-str))))
;;;
;;; (define-function (check-field part-list main-list cmp name err-str)
;;;   (srfi-5:let lp ((part part-list) (main main-list))
;;;     (if (null? part)
;;; 	main
;;; 	(if (null? main)
;;; 	    (error 'define-lambda-object err-str name (car part))
;;; 	    (let ((field (car part)))
;;; 	      (if (funcall cmp field (car main))
;;; 		  (lp (cdr part) (cdr main))
;;; 		  (srfi-5:let loop ((head (list (car main))) (tail (cdr main)))
;;; 		    (if (null? tail)
;;; 			(error 'define-lambda-object err-str name field)
;;; 			(if (funcall cmp field (car tail))
...
;;; 	      (let ((group (object :false :false :false)))
...
(define-function (field-key z k d)
  (let ((x (car z)) (y (cdr z)))
    (if (null? y)
	(cons d z)
	(if (eq? k x)
	    y
	    (iterate lp ((head (list x (car y))) (tail (cdr y)))
	      (if (null? tail)
		  (cons d z)
		  (let ((x (car tail)) (y (cdr tail)))
		    (if (null? y)
...
(define-function (check-field part-list main-list cmp name err-str)
  (iterate lp ((part part-list) (main main-list))
    (if (null? part)
	main
	(if (null? main)
	    (error 'define-lambda-object err-str name (car part))
	    (let ((field (car part)))
	      (if (funcall cmp field (car main))
		  (lp (cdr part) (cdr main))
		  (iterate loop ((head (list (car main))) (tail (cdr main)))
		    (if (null? tail)
			(error 'define-lambda-object err-str name field)
			(if (funcall cmp field (car tail))
			    (lp (cdr part) (append head (cdr tail)))
...
(defmacro define-object (name gr gi fm fi r o a c v h)
  (let ((safe-name (gentemp ".safe-name. "))
	(safe-parent (gentemp ".safe-parent. "))
	(arg (gensym "ARG-"))
	(args (gensym "ARGS-"))
	(makers (gentemp ".makers. "))
	;; (alist-a (gensym))
	;; (alist-m (gensym))
	;; (array (gensym))
	;; (safe-eq (gensym))
         ...
         (setq ,safe-parent
               (progn
                 ...
                 (for-each (lambda (g y)
                           ...
                           (list ,@(map (lambda (g) `#',g) gi)) ',gi)
                 (for-each (lambda (g y)
                             (flet ((g (&rest args)
                                      (apply g args) ))
                               (check-field (append (g :read-write-field)
                                                    (g :read-only-field)
                                                    (map #'car (g :hidden-field)) )
                                            ',(append fm (map #'car fi)
                                                      (map #'car h) )
                                            #'eq? y "incompatible whole field") ))
                           (list ,@(map (lambda (g) `#',g) gr)) ',gr)
                 (list ,@(map (lambda (g) `#',g) gi)
                       ,@(map (lambda (g) `#',g) gr) ) ))
         (setq ,makers
               (let* ,c
                 (cons (seq-lambda ,r ,o
                  ...
                  (or (eq? ,safe-name group)
                      (labels ((lp (group-list)
                                 (if (null? group-list)
                                     'NIL
                                     (or (eq? ,safe-name (car group-list))
                                         (lp (funcall (car group-list) :parent))
                                         (lp (cdr group-list)) ))))
                        (lp (funcall group :parent)) )))))
...
(define-function (field-sort gr gi field fm fi r o a c v h)
  (if (null? field)
      ...
      (let ((vars (car field)))
        (if (symbol? vars)		;r
            (if (and (null? o) (null? a) (null? c) (null? v))
                ...
                (error 'define-lambda-object
                       "required-field should precede optional-field and automatic-field"
                       vars) )
            (let ((var (car vars)))
              (if (symbol? var)
                  (if (null? (cdr vars)) ;(r)
                      ...
                      (if (null? (cddr vars)) ;(o val)
                          (if (and (null? a) (null? c) (null? v))
                              ...
                              (error 'define-lambda-object
                                     "optional-field should precede automatic-field"
                                     var) )
                          (error 'define-lambda-object
                                 "incorrect syntax"
                                 vars) ))
...
(define-function (group-sort gr gi gg field)
  (if (pair? gg)
      (let ((g (car gg)))
        (if (pair? g)
